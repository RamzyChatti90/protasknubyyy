...
type RestOf<T extends ITask | NewTask> = Omit<T, 'dueDate' | 'completedAt'> & {
  dueDate?: string | null;
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...
      completedAt: task.completedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...
      completedAt: restTask.completedAt ? dayjs(restTask.completedAt) : undefined,
    };
  }
